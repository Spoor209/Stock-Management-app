		...
		for(var index = 0; index < json.length; index++) {
			detailHtml += 
						"<tr>"+
							...
							"<td>" + json[index].article.prixUnitaire + "</td>"+
						"</tr>";
		}
				...
				if($("#qte" + data.article.idArticle).length > 0 && $("#total" + data.article.idArticle).length > 0) {
					...
					detailHtml += 
						"<tr id='ligne" + data.article.idArticle + "'>"+
							...
							"<td ><button class='fas fa-trash btn-circle btn-sm' onclick='supprimerLigneCommande(" + data.article.idArticle + ")'><i class='fas fa-trash'></i></button></td>"+
						"</tr>";
